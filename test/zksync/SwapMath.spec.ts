...
describe('BitMath', () => {
    ...
    const fixture = async () => {
        ...
        const artifact = await deployer.loadArtifact("SwapMathTest");
        const sqrtPriceMathTestFactoryArtifact = await deployer.loadArtifact("SqrtPriceMathTest")
        swapMath = (await deployer.deploy(artifact, [])) as SwapMathTest;
        sqrtPriceMath = (await deployer.deploy(sqrtPriceMathTestFactoryArtifact, [])) as SqrtPriceMathTest;
        // const factory = await ethers.getContractFactory('SwapMathTest')

        // return (await factory.deploy()) as SwapMathTest
    }
    before('deploy SwapMathTest', async () => {
