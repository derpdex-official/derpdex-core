...
describe('UniswapV3Pool', () => {
  ...
  beforeEach('deploy first fixture', async () => {
    ...
    const createPoolWrapped = async (
      ...
      const poolFunctions = await createPoolFunctions({
  ...
  describe('multi-swaps', () => {
    ...
    beforeEach('initialize both pools', async () => {
      ...
      tx = await pool0.initialize(encodePriceSqrt(1, 1))
      await tx.wait()
      tx = await pool1.initialize(encodePriceSqrt(1, 1))
      await tx.wait()

      tx = await pool0Functions.mint(wallet.address, minTick, maxTick, expandTo18Decimals(1))
      await tx.wait()
      tx = await pool1Functions.mint(wallet.address, minTick, maxTick, expandTo18Decimals(1))
      await tx.wait()
    })
