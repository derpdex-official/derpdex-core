...
//   let loadFixture: ReturnType<typeof waffle.createFixtureLoader>
  before('create fixture loader', async () => {
    ...
    wallet = new Wallet(PRIVATE_KEY, Provider.getDefaultProvider())
  })
  ...
  const noDelegateCallFixture = async () => {
    ...
    const artifact = await deployer.loadArtifact("NoDelegateCallTest")
    const noDelegateCallTest = await deployer.deploy(artifact) as NoDelegateCallTest
    const noDelegateCallTestFactory = new ContractFactory(artifact.abi, artifact.bytecode, wallet)
    const minimalProxyFactory = new ContractFactory(
