      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            tx = await fix.token0.transfer(arbitrageur.address, BigNumber.from(2).pow(254))
            await tx.wait()
            tx = await fix.token1.transfer(arbitrageur.address, BigNumber.from(2).pow(254))
            await tx.wait()
            ...
            await tx.wait()
            tx = await fix.token1.approve(tester.address, MaxUint256)
            await tx.wait()

            tx = await pool.initialize(startingPrice)
            await tx.wait()
            if (feeProtocol != 0) {
              tx = await pool.setFeeProtocol(feeProtocol, feeProtocol)
              await tx.wait()
            }
            tx = await mint(wallet.address, minTick, maxTick, passiveLiquidity)
            await tx.wait()
          ...
          for (const { zeroForOne, assumedTruePriceAfterSwap, inputAmount, description } of [
            ...
            describe(description, () => {
              ...
              it('sandwiched with swap to execution price then mint max liquidity/target/burn max liquidity', async () => {
                ...
                tx = await pool.burn(tickLower, tickUpper, getMaxLiquidityPerTick(tickSpacing))
                await tx.wait()
              ...
              it('backrun to true price after swap only', async () => {
                ...
                //   : await swapExact1For0(inputAmount, wallet.address)
                if (zeroForOne) {
                  tx = await swapExact0For1(inputAmount, wallet.address)
                } else {
                  tx = await swapExact1For0(inputAmount, wallet.address)
                }
                await tx.wait()
                ...
                if (zeroForOne) {
                  tx = await swapToHigherPrice(priceToSwapTo, wallet.address)
                } else {
                  tx = await swapToLowerPrice(priceToSwapTo, wallet.address)
                }
                await tx.wait()
                arbBalance0 = arbBalance0.sub(backrunDelta0)
                arbBalance1 = arbBalance1.sub(backrunDelta1)
