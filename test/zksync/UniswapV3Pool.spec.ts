...
describe('UniswapV3Pool', () => {
  ...
  describe('#initialize', () => {
    it('fails if already initialized', async () => {
      const tx = await pool.initialize(encodePriceSqrt(1, 1))
      await tx.wait()
  ...
  describe('#increaseObservationCardinalityNext', () => {
    ...
    it('does not change cardinality next if less than current', async () => {
      ...
      tx = await pool.increaseObservationCardinalityNext(3)
      await tx.wait()
      tx = await pool.increaseObservationCardinalityNext(2)
      await tx.wait()
  ...
  describe('#mint', () => {
    ...
    describe('after initialization', () => {
      beforeEach('initialize the pool at price of 10:1', async () => {
        tx = await pool.initialize(encodePriceSqrt(1, 10))
        await tx.wait()
        tx = await mint(wallet.address, minTick, maxTick, 3161)
        await tx.wait()
      })
      ...
      describe('success cases', () => {
        ...
        describe('above current price', () => {
          ...
          it('max tick with max leverage', async () => {
            tx = await mint(wallet.address, maxTick - tickSpacing, maxTick, BigNumber.from(2).pow(102))
            await tx.wait()
          ...
          it('removing works', async () => {
            tx = await mint(wallet.address, -240, 0, 10000)
            await tx.wait()
            tx = await pool.burn(-240, 0, 10000)
            await tx.wait()
          ...
          it('adds liquidity to liquidityGross', async () => {
            tx = await mint(wallet.address, -240, 0, 100)
            await tx.wait()
          ...
          it('removes liquidity from liquidityGross', async () => {
            tx = await mint(wallet.address, -240, 0, 100)
            await tx.wait()
            tx = await mint(wallet.address, -240, 0, 40)
            await tx.wait()
            tx = await pool.burn(-240, 0, 90)
            await tx.wait()
          ...
          it('only clears the tick that is not used at all', async () => {
            tx = await mint(wallet.address, -240, 0, 100)
            await tx.wait()
            tx = await mint(wallet.address, -tickSpacing, 0, 250)
            await tx.wait()
            tx = await pool.burn(-240, 0, 100)
            await tx.wait()
        ...
        describe('including current price', () => {
          ...
          it('initializes lower tick', async () => {
            tx = await mint(wallet.address, minTick + tickSpacing, maxTick - tickSpacing, 100)
            await tx.wait()
          ...
          it('removing works', async () => {
            tx = await mint(wallet.address, minTick + tickSpacing, maxTick - tickSpacing, 100)
            await tx.wait()
            tx = await pool.burn(minTick + tickSpacing, maxTick - tickSpacing, 100)
            await tx.wait()
          ...
          it('writes an observation', async () => {
            ...
            tx = await pool.advanceTime(1)
            await tx.wait()
            tx = await mint(wallet.address, minTick, maxTick, 100)
            await tx.wait()
            checkObservationEquals(await pool.observations(0), {
        ...
        describe('below current price', () => {
          ...
          it('min tick with max leverage', async () => {
            tx = await mint(wallet.address, minTick, minTick + tickSpacing, BigNumber.from(2).pow(102))
            await tx.wait()
            expect(await token0.balanceOf(pool.address)).to.eq(9996)
          ...
          it('removing works', async () => {
            tx = await mint(wallet.address, -46080, -46020, 10000)
            await tx.wait()
            tx = await pool.burn(-46080, -46020, 10000)
            await tx.wait()
            const { amount0, amount1 } = await pool.callStatic.collect(
              wallet.address,
              -46080,
          ...
          it('does not write an observation', async () => {
            ...
            tx = await pool.advanceTime(1)
            await tx.wait()
            tx = await mint(wallet.address, -46080, -23040, 100)
            await tx.wait()
            checkObservationEquals(await pool.observations(0), {
              tickCumulative: 0,
              blockTimestamp: TEST_POOL_START_TIME,
      ...
      it('protocol fees accumulate as expected during swap', async () => {
        tx = await pool.setFeeProtocol(6, 6)
        await tx.wait()

        tx = await mint(wallet.address, minTick + tickSpacing, maxTick - tickSpacing, expandTo18Decimals(1))
        await tx.wait()
        tx = await swapExact0For1(expandTo18Decimals(1).div(10), wallet.address)
        await tx.wait()
        tx = await swapExact1For0(expandTo18Decimals(1).div(100), wallet.address)
        await tx.wait()
      ...
      it('poke is not allowed on uninitialized position', async () => {
        tx = await mint(other.address, minTick + tickSpacing, maxTick - tickSpacing, expandTo18Decimals(1))
        await tx.wait()
        tx = await swapExact0For1(expandTo18Decimals(1).div(10), wallet.address)
        await tx.wait()
        tx = await swapExact1For0(expandTo18Decimals(1).div(100), wallet.address)
        ...
        tx = await mint(wallet.address, minTick + tickSpacing, maxTick - tickSpacing, 1)
        await tx.wait()
        let {
          liquidity,
        ...
        tx = await pool.burn(minTick + tickSpacing, maxTick - tickSpacing, 1)
        await tx.wait()
        ;({
          liquidity,
          feeGrowthInside0LastX128,
  ...
  describe('#burn', () => {
    ...
    it('does not clear the position fee growth snapshot if no more liquidity', async () => {
      ...
      tx = await pool.advanceTime(10)
      await tx.wait()
      tx = await mint(other.address, minTick, maxTick, expandTo18Decimals(1))
      await tx.wait()
      tx = await swapExact0For1(expandTo18Decimals(1), wallet.address)
      await tx.wait()
      tx = await swapExact1For0(expandTo18Decimals(1), wallet.address)
      await tx.wait()
      tx = await pool.connect(other as unknown as Signer).burn(minTick, maxTick, expandTo18Decimals(1))
      await tx.wait()
      const {
        liquidity,
        tokensOwed0,
    ...
    it('clears the tick if its the last position using it', async () => {
      ...
      tx = await pool.advanceTime(10)
      await tx.wait()
      tx = await mint(wallet.address, tickLower, tickUpper, 1)
      await tx.wait()
      tx = await swapExact0For1(expandTo18Decimals(1), wallet.address)
      await tx.wait()
      tx = await pool.burn(tickLower, tickUpper, 1)
      await tx.wait()
    ...
    it('clears only the lower tick if upper is still used', async () => {
      ...
      tx = await mint(wallet.address, tickLower, tickUpper, 1)
      await tx.wait()
      tx = await mint(wallet.address, tickLower + tickSpacing, tickUpper, 1)
      await tx.wait()
      tx = await swapExact0For1(expandTo18Decimals(1), wallet.address)
      await tx.wait()
      tx = await pool.burn(tickLower, tickUpper, 1)
    ...
    it('clears only the upper tick if lower is still used', async () => {
      ...
      tx = await mint(wallet.address, tickLower, tickUpper, 1)
      await tx.wait()
      tx = await mint(wallet.address, tickLower, tickUpper - tickSpacing, 1)
      await tx.wait()
      tx = await swapExact0For1(expandTo18Decimals(1), wallet.address)
      await tx.wait()
      tx = await pool.burn(tickLower, tickUpper, 1)
  ...
  async function initializeAtZeroTick(pool: MockTimeUniswapV3Pool): Promise<void> {
    ...
    tx = await mint(wallet.address, min, max, initializeLiquidityAmount)
    await tx.wait()
  }
  ...
  describe('#observe', () => {
    ...
    it('current tick accumulator after two swaps', async () => {
      tx = await swapExact0For1(expandTo18Decimals(1).div(2), wallet.address)
      await tx.wait()
      expect((await pool.slot0()).tick).to.eq(-4452)
      tx = await pool.advanceTime(4)
      await tx.wait()
      tx = await swapExact1For0(expandTo18Decimals(1).div(4), wallet.address)
      await tx.wait()
      expect((await pool.slot0()).tick).to.eq(-1558)
      tx = await pool.advanceTime(6)
      await tx.wait()
      let {
        tickCumulatives: [tickCumulative],
      } = await pool.observe([0])
  ...
  describe('miscellaneous mint tests', () => {
    ...
    it('mint to the right of the current price', async () => {
      ...
      tx = await mint(wallet.address, lowerTick, upperTick, liquidityDelta)
      await tx.wait()
    ...
    it('cannot remove more than the entire position', async () => {
      const lowerTick = -tickSpacing
      const upperTick = tickSpacing
      tx = await mint(wallet.address, lowerTick, upperTick, expandTo18Decimals(1000))
      await tx.wait()
    ...
    it('collect fees within the current price after swap', async () => {
      ...
      tx = await swapExact0For1(amount0In, wallet.address)
      await tx.wait()
      ...
      tx = await pool.burn(lowerTick, upperTick, 0)
      await tx.wait()
      tx = await pool.collect(wallet.address, lowerTick, upperTick, MaxUint128, MaxUint128)
      await tx.wait()
    ...
    describe('k (implicit)', () => {
      ...
      describe('post initialized', () => {
        ...
        it('returns in supply in range', async () => {
          tx = await mint(wallet.address, -tickSpacing, tickSpacing, expandTo18Decimals(3))
          await tx.wait()
          expect(await pool.liquidity()).to.eq(expandTo18Decimals(5))
        })
        it('excludes supply at tick above current tick', async () => {
          tx = await mint(wallet.address, tickSpacing, tickSpacing * 2, expandTo18Decimals(3))
          await tx.wait()
          expect(await pool.liquidity()).to.eq(expandTo18Decimals(2))
        })
        it('excludes supply at tick below current tick', async () => {
          tx = await mint(wallet.address, -tickSpacing * 2, -tickSpacing, expandTo18Decimals(3))
          await tx.wait()
          expect(await pool.liquidity()).to.eq(expandTo18Decimals(2))
        })
        it('updates correctly when exiting range', async () => {
          ...
          tx = await swapExact0For1(1, wallet.address)
          await tx.wait()
  ...
  describe('limit orders', () => {
    ...
    it('limit selling 0 for 1 at tick 0 thru 1', async () => {
      ...
      tx = await swapExact1For0(expandTo18Decimals(2), other.address)
      await tx.wait()
    ...
    it('limit selling 1 for 0 at tick 0 thru -1', async () => {
      await expect(mint(wallet.address, -120, 0, expandTo18Decimals(1)))
        ...
        .withArgs(wallet.address, pool.address, '5981737760509663')
      // somebody takes the limit order
      tx = await swapExact0For1(expandTo18Decimals(2), other.address)
      await tx.wait()
  ...
  describe('#collect', () => {
    ...
    it('works with multiple LPs', async () => {
      tx = await mint(wallet.address, minTick, maxTick, expandTo18Decimals(1))
      await tx.wait()
      tx = await mint(wallet.address, minTick + tickSpacing, maxTick - tickSpacing, expandTo18Decimals(2))
      await tx.wait()
      ...
      // poke positions
      tx = await pool.burn(minTick, maxTick, 0)
      await tx.wait()
      tx = await pool.burn(minTick + tickSpacing, maxTick - tickSpacing, 0)
      await tx.wait()
    ...
    describe('works across large increases', () => {
      ...
      it('works just before the cap binds', async () => {
        tx = await pool.setFeeGrowthGlobal0X128(magicNumber)
        await tx.wait()
        tx = await pool.burn(minTick, maxTick, 0)
        await tx.wait()
      ...
      it('works just after the cap binds', async () => {
        tx = await pool.setFeeGrowthGlobal0X128(magicNumber.add(1))
        await tx.wait()
        tx = await pool.burn(minTick, maxTick, 0)
        await tx.wait()
      ...
      it('works well after the cap binds', async () => {
        tx = await pool.setFeeGrowthGlobal0X128(constants.MaxUint256)
        await tx.wait()
        tx = await pool.burn(minTick, maxTick, 0)
        await tx.wait()
    ...
    describe('works across overflow boundaries', () => {
      beforeEach(async () => {
        tx = await pool.setFeeGrowthGlobal0X128(constants.MaxUint256)
        await tx.wait()
        tx = await pool.setFeeGrowthGlobal1X128(constants.MaxUint256)
        await tx.wait()
        tx = await mint(wallet.address, minTick, maxTick, expandTo18Decimals(10))
        await tx.wait()
      })
  ...
  describe('#feeProtocol', () => {
    ...
    beforeEach(async () => {
      ...
      tx = await pool.initialize(encodePriceSqrt(1, 1))
      await tx.wait()
      tx = await mint(wallet.address, minTick, maxTick, liquidityAmount)
      await tx.wait()
    })
    ...
    async function swapAndGetFeesOwed({
      ...
      tx = await (zeroForOne ? swapExact0For1(amount, wallet.address) : swapExact1For0(amount, wallet.address))
      await tx.wait()
    ...
    it('fees collected by lp after two swaps with intermediate withdrawal', async () => {
      ...
      // collect the fees
      tx = await pool.collect(wallet.address, minTick, maxTick, MaxUint128, MaxUint128)
      await tx.wait()
      ...
      tx = await pool.burn(minTick, maxTick, 0) // poke to update fees
      await tx.wait()
    ...
    describe('tickSpacing = 12', () => {
      ...
      describe('post initialize', () => {
        ...
        it('mint can be called with multiples of 12', async () => {
          tx = await mint(wallet.address, 12, 24, 1)
          await tx.wait()
          tx = await mint(wallet.address, -144, -120, 1)
          await tx.wait()
        })
        it('swapping across gaps works in 1 for 0 direction', async () => {
          const liquidityAmount = expandTo18Decimals(1).div(4)
          tx = await mint(wallet.address, 120000, 121200, liquidityAmount)
          await tx.wait()
          tx = await swapExact1For0(expandTo18Decimals(1), wallet.address)
          await tx.wait()
        ...
        it('swapping across gaps works in 0 for 1 direction', async () => {
          const liquidityAmount = expandTo18Decimals(1).div(4)
          tx = await mint(wallet.address, -121200, -120000, liquidityAmount)
          await tx.wait()
          tx = await swapExact0For1(expandTo18Decimals(1), wallet.address)
          await tx.wait()
  ...
  it('tick transition cannot run twice if zero for one swap ends at fractional price just below tick', async () => {
    ...
    tx = await mint(wallet.address, -24082, -24080, liquidity)
    await tx.wait()
    expect(await pool.liquidity(), 'current pool liquidity is now liquidity + 1').to.eq(liquidity)

    tx = await mint(wallet.address, -24082, -24081, liquidity)
    await tx.wait()
    ...
    describe('after liquidity added', () => {
      ...
      describe('fee off', () => {
        ...
        it('increases the fee growth by the expected amount', async () => {
          tx = await flash(1001, 2002, other.address)
          await tx.wait()
      ...
      describe('fee on', () => {
        ...
        it('increases the fee growth by the expected amount', async () => {
          tx = await flash(2002, 4004, other.address)
          await tx.wait()
  ...
  describe('#setFeeProtocol', () => {
    ...
    it('succeeds for fee of 4', async () => {
      tx = await pool.setFeeProtocol(4, 4)
      await tx.wait()
    })
    it('succeeds for fee of 10', async () => {
      tx = await pool.setFeeProtocol(10, 10)
      await tx.wait()
    })
    it('sets protocol fee', async () => {
      tx = await pool.setFeeProtocol(7, 7)
      await tx.wait()
    ...
    it('can change protocol fee', async () => {
      tx = await pool.setFeeProtocol(7, 7)
      await tx.wait()
      tx = await pool.setFeeProtocol(5, 8)
      await tx.wait()
    ...
    it('can turn off protocol fee', async () => {
      tx = await pool.setFeeProtocol(4, 4)
      await tx.wait()
      tx = await pool.setFeeProtocol(0, 0)
      await tx.wait()
      expect((await pool.slot0()).feeProtocol).to.eq(0)
    })
    ...
    it('emits an event when unchanged', async () => {
      tx = await pool.setFeeProtocol(5, 9)
      await tx.wait()
  ...
  describe('#snapshotCumulativesInside', () => {
    ...
    beforeEach(async () => {
      tx = await pool.initialize(encodePriceSqrt(1, 1))
      await tx.wait()
      tx = await mint(wallet.address, tickLower, tickUpper, 10)
      await tx.wait()
    })
    ...
    it('does not account for time increase above range', async () => {
      tx = await pool.advanceTime(5)
      await tx.wait()
      tx = await swapToHigherPrice(encodePriceSqrt(2, 1), wallet.address)
      await tx.wait()
      tx = await pool.advanceTime(7)
      await tx.wait()
      const {
        secondsPerLiquidityInsideX128,
        tickCumulativeInside,
    ...
    it('does not account for time increase below range', async () => {
      tx = await pool.advanceTime(5)
      await tx.wait()
      tx = await swapToLowerPrice(encodePriceSqrt(1, 2), wallet.address)
      await tx.wait()
      tx = await pool.advanceTime(7)
      await tx.wait()
      const {
    ...
    it('time increase below range is not counted', async () => {
      ...
      tx = await pool.advanceTime(5)
      await tx.wait()
      tx = await swapToHigherPrice(encodePriceSqrt(1, 1), wallet.address)
      await tx.wait()
      tx = await pool.advanceTime(7)
      await tx.wait()
      const {
    ...
    it('time increase above range is not counted', async () => {
      ...
      tx = await pool.advanceTime(5)
      await tx.wait()
      tx = await swapToLowerPrice(encodePriceSqrt(1, 1), wallet.address)
      await tx.wait()
      tx = await pool.advanceTime(7)
      await tx.wait()
      const {
    ...
    it('positions minted after time spent', async () => {
      tx = await pool.advanceTime(5)
      await tx.wait()
      tx = await mint(wallet.address, tickUpper, getMaxTick(tickSpacing), 15)
      await tx.wait()
      tx = await swapToHigherPrice(encodePriceSqrt(2, 1), wallet.address)
      await tx.wait()
      tx = await pool.advanceTime(8)
      await tx.wait()
      const {
        secondsPerLiquidityInsideX128,
        tickCumulativeInside,
    ...
    it('overlapping liquidity is aggregated', async () => {
      tx = await mint(wallet.address, tickLower, getMaxTick(tickSpacing), 15)
      await tx.wait()
      tx = await pool.advanceTime(5)
      await tx.wait()
      tx = await swapToHigherPrice(encodePriceSqrt(2, 1), wallet.address)
    ...
    it('relative behavior of snapshots', async () => {
      tx = await pool.advanceTime(5)
      await tx.wait()
      tx = await mint(wallet.address, getMinTick(tickSpacing), tickLower, 15)
      await tx.wait()
      const {
  ...
  describe('fees overflow scenarios', async () => {
    it('up to max uint 128', async () => {
      await pool.initialize(encodePriceSqrt(1, 1))
      tx = await mint(wallet.address, minTick, maxTick, 1)
      await tx.wait()
      tx = await flash(0, 0, wallet.address, MaxUint128, MaxUint128)
      await tx.wait()
    ...
    it('overflow max uint 128', async () => {
      ...
      tx = await flash(0, 0, wallet.address, MaxUint128, MaxUint128)
      await tx.wait()
      tx = await flash(0, 0, wallet.address, 1, 1)
      await tx.wait()
    ...
    it('two positions at the same snapshot', async () => {
      ...
      tx = await mint(wallet.address, minTick, maxTick, 1)
      await tx.wait()
      tx = await mint(other.address, minTick, maxTick, 1)
      await tx.wait()
      tx = await flash(0, 0, wallet.address, MaxUint128, 0)
      await tx.wait()
      tx = await flash(0, 0, wallet.address, MaxUint128, 0)
      await tx.wait()
      const feeGrowthGlobal0X128 = await pool.feeGrowthGlobal0X128()
      expect(feeGrowthGlobal0X128).to.eq(MaxUint128.shl(128))
      tx = await flash(0, 0, wallet.address, 2, 0)
      await tx.wait()
      tx = await pool.burn(minTick, maxTick, 0)
      await tx.wait()
      tx = await pool.connect(other as unknown as Signer).burn(minTick, maxTick, 0)
      await tx.wait()
    ...
    it('two positions 1 wei of fees apart overflows exactly once', async () => {
      ...
      tx = await mint(wallet.address, minTick, maxTick, 1)
      await tx.wait()
      tx = await flash(0, 0, wallet.address, 1, 0)
      await tx.wait()
      tx = await mint(other.address, minTick, maxTick, 1)
      await tx.wait()
      tx = await flash(0, 0, wallet.address, MaxUint128, 0)
