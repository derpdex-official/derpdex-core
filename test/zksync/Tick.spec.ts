...
describe('Tick', () => {
  ...
  describe('#getFeeGrowthInside', () => {
    ...
    it('subtracts upper tick if below', async () => {
      const tx = await tickTest.setTick(2, {
  ...
  describe('#update', async () => {
    ...
    it('does not flip from nonzero to greater nonzero', async () => {
      const tx = await tickTest.update(0, 0, 1, 0, 0, 0, 0, 0, false, 3)
      await tx.wait()
    ...
    it('does not flip from nonzero to lesser nonzero', async () => {
      const tx = await tickTest.update(0, 0, 2, 0, 0, 0, 0, 0, false, 3)
      await tx.wait()
      expect(await tickTest.callStatic.update(0, 0, -1, 0, 0, 0, 0, 0, false, 3)).to.eq(false)
    })
    it('does not flip from nonzero to lesser nonzero', async () => {
    ...
    it('reverts if total liquidity gross is greater than max', async () => {
      let tx
      tx = await tickTest.update(0, 0, 2, 0, 0, 0, 0, 0, false, 3)
      await tx.wait()
      tx = await tickTest.update(0, 0, 1, 0, 0, 0, 0, 0, true, 3)
      await tx.wait()
    ...
    it('nets the liquidity based on upper flag', async () => {
      let tx
      tx = await tickTest.update(0, 0, 2, 0, 0, 0, 0, 0, false, 10)
      await tx.wait()
      tx = await tickTest.update(0, 0, 1, 0, 0, 0, 0, 0, true, 10)
      await tx.wait()
      tx = await tickTest.update(0, 0, 3, 0, 0, 0, 0, 0, true, 10)
      await tx.wait()
      tx = await tickTest.update(0, 0, 1, 0, 0, 0, 0, 0, false, 10)
      await tx.wait()
    ...
    it('reverts on overflow liquidity gross', async () => {
      const tx = await tickTest.update(0, 0, MaxUint128.div(2).sub(1), 0, 0, 0, 0, 0, false, MaxUint128)
      await tx.wait()
    ...
    it('assumes all growth happens below ticks lte current tick', async () => {
      const tx = await tickTest.update(1, 1, 1, 1, 2, 3, 4, 5, false, MaxUint128)
      await tx.wait()
      const {
    ...
    it('does not set any growth fields if tick is already initialized', async () => {
      let tx
      tx = await tickTest.update(1, 1, 1, 1, 2, 3, 4, 5, false, MaxUint128)
      await tx.wait()
      tx = await tickTest.update(1, 1, 1, 6, 7, 8, 9, 10, false, MaxUint128)
      await tx.wait()
      const {
        feeGrowthOutside0X128,
        feeGrowthOutside1X128,
    ...
    it('does not set any growth fields for ticks gt current tick', async () => {
      const tx = await tickTest.update(2, 1, 1, 1, 2, 3, 4, 5, false, MaxUint128)
      await tx.wait()
      const {
        feeGrowthOutside0X128,
        feeGrowthOutside1X128,
  ...
  describe('#cross', () => {
    it('flips the growth variables', async () => {
      ...
      await tx.wait()
      tx = await tickTest.cross(2, 7, 9, 8, 15, 10)
      await tx.wait()
      const {
        feeGrowthOutside0X128,
        feeGrowthOutside1X128,
    ...
    it('two flips are no op', async () => {
      ...
      await tx.wait()
      tx = tx = await tickTest.cross(2, 7, 9, 8, 15, 10)
      await tx.wait()
      tx = tx = await tickTest.cross(2, 7, 9, 8, 15, 10)
      await tx.wait()
      const {
